...
# Carrega as imagens
try:
    # Substitua "jogador.png" pelo nome do seu arquivo de imagem
    player_img = pygame.image.load("jogador.png").convert_alpha()
    # Redimensiona a imagem se necessário (opcional)
    player_img = pygame.transform.scale(player_img, (60, 100))
except:
    print("Erro ao carregar imagem do jogador. Usando placeholder.")
    # Fallback para o jogador geométrico se a imagem não for encontrada
    player_img = pygame.Surface((60, 100), pygame.SRCALPHA)
    pygame.draw.ellipse(player_img, BLUE, (0, 0, 60, 100))
    pygame.draw.circle(player_img, WHITE, (30, 20), 20)
...
# Cria as superfícies da bola e do gol
...
while running:
    ...
    player_rect = pygame.Rect(player_pos[0], player_pos[1], 60, 100)  # Ajustado para o novo tamanho
    ball_rect = pygame.Rect(ball_pos[0], ball_pos[1], 30, 30)
    if check_collision(player_rect, ball_rect) and not ball_moving:
        # Empurra a bola
        ball_pos[0] = player_pos[0] + 70  # Ajustado para o novo tamanho
    ...
    # Desenha o jogador (agora com imagem)
